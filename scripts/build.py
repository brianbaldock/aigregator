...
def _og_image_dims(og_image: str) -> tuple[int, int, str]:
    """Return (width, height, mime) for a site-relative og:image.

    Read from the actual file rather than hardcoded, so the declared dimensions
    track the asset. Falls back to the known card geometry for absolute/remote
    URLs or if Pillow is unavailable -- the build must not break over a preview
    hint, but scripts/verify_seo.py separately FAILS if the declared values do
    not match the decoded image, so a wrong fallback cannot ship silently.
    """
    mime = "image/jpeg" if og_image.lower().endswith((".jpg", ".jpeg")) else "image/png"
    if og_image.startswith("http"):
        return 1200, 630, mime
    path = DOCS_DIR / og_image.lstrip("/")
    try:
        from PIL import Image  # local import: only the build needs it

        with Image.open(path) as im:
            return im.size[0], im.size[1], mime
    except Exception:
        return 1200, 630, mime
...
def html_shell(
    ...
    canonical_path: str = "",
    # 1200x630 baked card (tools/build_og_cover.py). The square logo previews
    # cropped or letterboxed on every platform and is 1.5MB; see verify_seo.py,
    # which gates the card's real dimensions rather than its mere existence.
    og_image: str = "assets/og-cover.jpg",
    extra_head: str = "",
    og_type: str = "website",
    noindex: bool = False,
) -> str:
    ...
    # Declare the card's REAL decoded dimensions, read from the file on disk, so
    # the declared size cannot drift from the asset (a stale width/height makes
    # platforms reserve the wrong box and reflow the preview).
    og_w, og_h, og_mime = _og_image_dims(og_image)
...
<meta property="og:image:alt" content="AIGREGATOR — daily AI news digest, scored, clustered and cited">
...
def _site_ld(extra: list[dict] | None = None) -> str:
    """JSON-LD for the shell pages (index, archive, about, weekly index).

    Only claims things that are TRUE: this is a periodical publication with a
    search-free static archive, authored by one person and generated by an
    agent. No NewsMediaOrganization (implies an editorial org that does not
    exist), no fabricated ratings, no Person credentials we cannot support.
    Digest and weekly pages carry their own richer per-page blocks.
    """
    import json as _json
    ...
    blocks: list[dict] = [
        {
            ...
            "name": "AIgregator",
            "description": (
                "Daily AI news digest, scored and cited. Generated each "
                "morning by an autonomous Hermes agent."
            ),
            "inLanguage": "en",
        ...
        },
        {
            ...
            "name": "Brian Baldock",
            "url": "https://www.linkedin.com/in/bbaldock",
        },
    ]
    blocks.extend(extra or [])
    return "\n".join(
        f'<script type="application/ld+json">{_json.dumps(b)}</script>' for b in blocks
    )
        ...
        extra_head=_site_ld([
            {
                ...
                "name": "AIgregator archive",
                "isPartOf": {"@id": f"{SITE_URL}/#website"},
                # Count is derived from the same `entries` the table renders, so
                # the claim cannot drift from the page.
                "mainEntity": {
                    "@type": "ItemList",
                    ...
                    "itemListElement": [
                        {
                            "@type": "ListItem",
                            "position": i,
                            "url": f"{SITE_URL}/digests/{e['slug']}.html",
                            "name": e.get("title") or e["slug"],
                        }
...
def build_weekly() -> list[dict]:
    ...
    for md in sorted(WEEKLY_DIR.glob("*.md"), reverse=True):
        ...
        # Social card: the 1200x630 weekly card, NOT the square banner PNG.
        # The banner still renders in-page (per-theme swap handled in app.js);
        # a 1:1 1.5MB image as og:image crops or letterboxes on every platform.
        og_img = "assets/og-cover-weekly.jpg"
        body = f"""
...
"""
        idx_page = html_shell(
            ...
            og_image="assets/og-cover-weekly.jpg",
            extra_head='<meta name="aig:page-type" content="weekly">\n' + _site_ld(),
        )
